...
<template>
  <section aria-live="polite" :aria-busy="loading">
    ...
    <ul v-else class="m-0 grid list-none grid-cols-1 gap-3 p-0 md:grid-cols-2 lg:grid-cols-3">
      <li v-for="dog in dogs" :key="dog._id" class="dog-card min-h-34 rounded-[14px] border border-[var(--line)] bg-[var(--surface)] p-4 shadow-[0_2px_5px_rgba(20,33,61,.02)] transition hover:-translate-y-0.5 hover:shadow-[var(--shadow)]">
        ...
        <div v-if="dog.subBreeds.length" class="dog-card-tags mt-3.5 flex flex-wrap gap-1.5">
